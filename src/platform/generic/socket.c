...
src/platform/generic/socket.c - POSIX socket functions
                              ...
                              ARGMOD(INTVAL *value))
{
    ...
    return getsockopt((PIOSOCKET)handle, level, &option, value, sizeof(value));
}
                              ...
                              ARGIN(INTVAL value))
{
    PARROT_SOL_LEVEL(level);
    PARROT_SO_OPTION(option);
    return setsockopt((PIOSOCKET)handle, level, &option, &value, sizeof(value));
}
